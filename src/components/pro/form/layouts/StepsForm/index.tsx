...
import type { FormInstance, StepsProps } from '@/components/base'
import {
  Col,
  ConfigProvider,
  Form,
...
import type { FormProviderProps } from '@/components/base/form/context'
import { compareVersions, merge, useRefFunction } from '../../../utils'
...
function StepsForm<T = Record<string, any>>(
  ...
  /**
   * ProForm處理了一下 from 的資料，在其中做了一些操作 如果使用 Provider 自帶的，自帶的資料處理就無法生效了
   */
  ...
  const next = useMemo(() => {
    return (
      submitter !== false && (
        ...
        >
          {intl.formatMessage({ id: 'stepsForm.next', defaultMessage: '下一步' })}
        </LoadingButton>
      )
    )
  ...
  const pre = useMemo(() => {
    return (
      submitter !== false && (
        ...
        >
          {intl.formatMessage({ id: 'stepsForm.prev', defaultMessage: '上一步' })}
        </LoadingButton>
      )
    )
  ...
  const submit = useMemo(() => {
    return (
      submitter !== false && (
        ...
        >
          {intl.formatMessage({ id: 'stepsForm.submit', defaultMessage: '提交' })}
        </LoadingButton>
      )
    )
  }, [intl, loading, onSubmit, submitter])
