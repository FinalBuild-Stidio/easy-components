import { Keyframes } from '@/components/StyleContext'
  ...
  return {
    [`${token.proComponentsCls}-layout`]: {
      ...
      [`${token.componentCls}${token.componentCls}-fixed`]: {
        ...
        height: '100%',
        '&-mix': {
          height: `calc(100% - ${token.layout?.header?.heightLayoutHeader || 56
            }px)`,
          insetBlockStart: `${token.layout?.header?.heightLayoutHeader || 56
            }px`,
        },
      },
    },
...
export function useStyle(
  ...
  return useIpassStyle('ProLayoutSiderMenu', (token) => {
