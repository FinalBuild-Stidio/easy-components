...
/**
 * 獲取locale，增加了一個功能，如果 locale = false，將不使用國際化
 * @param item
 * @param parentName
 */
const getItemLocaleName = (
  ...
  // 如果配置了 locale 並且 locale 為 false或 ""
...
const bigfishCompatibleConversions = (
  ...
  } = menu as Route // 相容平鋪式寫法
  // 拼接 childrenRoutes, 處理存在 indexRoute 時的邏輯
  const childrenList =
    indexRoute &&
      // 如果只有 redirect,不用處理的
...
/**
 * 刪除 hideInMenu 和 item.name 不存在的
 */
...
/**
 * 獲取麵包屑映射
 * @param MenuDataItem[] menuData 菜單配置
 */
