...
export interface SeedToken extends PresetColorType {
   ...
   * @desc 用於表示操作成功的 Token 序列，如 Result、Progress 等元件會使用該組變化變數。
   ...
   * @desc 用於表示操作警告的 Token 序列，如 Notification、 Alert等警告類元件或 Input 輸入類等元件會使用該組變化變數。
   ...
   * @nameZH 資訊色
   * @nameEN Info Color
   * @desc 用於表示操作資訊的 Token 序列，如 Alert 、Tag、 Progress 等元件都有用到該組變化變數。
   ...
   * @desc 用於派生文字色變化的基礎變數，v5 中我們新增了一層文字色的派生算法可以產出變化明確的文字色的變化變數。但請不要在程式碼中直接使用該 Seed Token ！
   ...
   * @desc 用於派生背景色變化的基礎變數，v5 中我們新增了一層背景色的派生算法可以產出變化明確的背景色的變化變數。但請不要在程式碼中直接使用該 Seed Token ！
   ...
   * @desc 設計系統中使用最廣泛的字體大小，文字變化也將基於該字號進行派生。
   ...
   * @desc 用於控制元件尺寸的基礎步長，尺寸步長結合尺寸變化單位，就可以派生各種尺寸變化。通過調整步長即可得到不同的布局模式，例如 V5 緊湊模式下的尺寸步長為 2
  ...
  //  ----------   motion   ---------- //
  // TODO: 缺一個懂 motion 的人來收斂 Motion 相關的 Token
