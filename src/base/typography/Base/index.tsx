...
const Base = React.forwardRef<HTMLElement, BlockProps>((props, ref) => {
  ...
  }, [enableEllipsis, cssEllipsis, children, cssLineClamp, isNativeVisible])  // Use IntersectionObserver to check if element is invisible
  React.useEffect(() => {
    const textEle = typographyRef.current
    if (
  ...
  const renderCopy = () => {
    ...
    return (
      <Tooltip key="copy" title={copyTitle}>
        ...
        >
          {copied
            ? getNode(iconNodes[1], <CheckIcon sx={{ fontSize: 14 }} />, true)
            : getNode(iconNodes[0], <ContentCopyIcon sx={{ fontSize: 14 }} />, true)}
        </TransButton>
      </Tooltip>
    )
  }
